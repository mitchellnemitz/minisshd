...
- Do not cite line numbers when referencing code locations in comments, commit messages, plans, or any other text. Line numbers shift the moment any line above the target is inserted or deleted, making the reference wrong without any warning. Use Go identifiers instead: `package.Symbol` for exported names (e.g. `internal/ratelimit.Backoff`, `server.Serve`), or `path/file.go: FunctionName` when the file path aids navigation but the symbol name is the stable anchor. For spec cross-references, the established `§N` section-number form is already the right choice.
