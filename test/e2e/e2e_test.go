...
// no MINISSH_PASS; parse the Password: line; that password works.
...
//
// Uses -o HostKeyAlias=minissh-test so the known_hosts entry is keyed
...
func TestE2E_HostKeyPermissionRefusal(t *testing.T) {
	...
	code, out := runMinisshOnce(t, spawnOptions{home: home})
	if code != 4 {
...
func TestE2E_InvalidBindAddress(t *testing.T) {
	...
	code, out := runMinisshOnce(t, spawnOptions{
		bind: "not-an-ip",
